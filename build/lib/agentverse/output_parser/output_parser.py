...
@output_parser_registry.register("nlp_meeting_3players")
class NlpMeeting3PlayersParser(OutputParser):
    def parse(self, output: LLMResult) -> Union[AgentAction, AgentFinish]:
        text = output.content
        cleaned_output = text.strip()
        cleaned_output = re.sub(r"\n+", "\n", cleaned_output)
        cleaned_output = cleaned_output.split("\n")
        # print(111,cleaned_output)
        if (
            len(cleaned_output) == 2
            and cleaned_output[0].startswith("Action:")
            and cleaned_output[1].startswith("Action Input:")
            ...
            action = cleaned_output[0][len("Action:") :].strip()
            action_input = cleaned_output[1][len("Action Input:") :].strip()
        elif ("Action:" in '\n'.join(cleaned_output) and "Action Input:" in '\n'.join(cleaned_output)):
            action = [t for t in cleaned_output if "Action:" in t][0]
            action = action[action.index("Action:")+len("Action:")+1:].strip().replace('(','').replace(')','')
            action_input = [t for t in cleaned_output if "Action Input:" in t][0]
            action_input = action_input[action_input.index("Action Input:")+len("Action Input:")+1:].strip()
        else:
            raise OutputParserError(text)
